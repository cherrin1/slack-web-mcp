...
app.post('/', async (req, res) => {
  ...
  try {
    ...
    if (method === 'initialize') {
      ...
      if (hasAuth) {
        console.log('🔧 Initialize has auth - returning capabilities with tools EMBEDDED');
        const initResponse = { 
          jsonrpc: '2.0',
          result: {
            ...
            // EXPERIMENTAL: Try including tools directly in initialize response
            tools: [
              { 
        ...
        console.log('🔧 Initialize response sent with tools capability AND embedded tools');
    ...
    // SPECIAL: Allow tools/list without auth for debugging
    if (method === 'tools/list') {
      console.log('🔧 TOOLS/LIST CALLED! (allowing without auth for debugging)');
      const toolsResponse = { 
        jsonrpc: '2.0',
        result: {
          tools: [
      ...
      console.log('🔧 Tools list sent successfully (no auth required):', toolsResponse.result.tools.length, 'tools');
